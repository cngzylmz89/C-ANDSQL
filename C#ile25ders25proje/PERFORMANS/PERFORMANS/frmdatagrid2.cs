    ...
    public partial class frmdatagrid2 : Form
    {
        ...
        { //ÖLÇÜT DATAGRID VİEW
            if(sec == 0)
            {
                ...
                OleDbDataAdapter da = new OleDbDataAdapter("SELECT OGRENCIADISOYADI, SUM([1_OLCUT]) AS 'BİRİNCİ ÖLÇÜT', SUM([2_OLCUT]) AS 'İKİNCİ ÖLÇÜT', SUM([3_OLCUT]) AS 'ÜÇÜNCÜ ÖLÇÜT', SUM([4_OLCUT]) AS 'DÖRDÜNCÜ ÖLÇÜT', SUM([5_OLCUT]) AS 'BEŞİNCİ ÖLÇÜT', SUM(TOPLAMPUAN) AS 'TOPLAM PUAN' FROM TBLNOTLAR  INNER JOIN  TBLOGRENCILER ON TBLOGRENCILER.OGRENCIID=TBLNOTLAR.OGRENCININID GROUP BY OGRENCIADISOYADI ORDER BY  SUM(TOPLAMPUAN) DESC ", con);
